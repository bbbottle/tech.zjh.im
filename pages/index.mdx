...
## 关于作者
zjh (b. 1993 China, based in ShenZhen, will move to ChangSha soon) is a web front end developer obsessed with React and has an interesting in back end techs too.

He enjoys coding and believes in being is more important than doing.
...
## 内容说明
[zjh.im](https://zjh.im) 具备博文发布功能，只是它像是自己用所学技术做给自己的礼物。更习惯于在上边发布生活随笔类的文字。尝试过在上边发布技术相关的文章，生活文字的随性矫情和技术文字的严肃冰冷总是发生奇怪地反应。显得不伦不类，索性分开写。因此与烟火关系不大且矫情指数较低的文字统一记录到这里。


## 持续部署
tech.zjh.im 是用 [nextra](https://nextra.vercel.app/) 生成的静态网站，主要内容由 mdx 文件构成。为了专注、高效地更新站点内容，构建部署这种重复性的动作应该交给 CD 完成。利用现成的 [ssh-action](https://github.com/appleboy/ssh-action) 远程连到 VPS 并执行相应的命令即可。
...
<Details name="Fig. 更新、发布流程">
  <Image src="/images/content-publish-flow.svg" width={931} height={1113} className="next-image" />
</Details>
